...
module.exports = class Database {
	...
	getPlayers(query)
	{
		...
		return collection.find(query).toArray().then(function(documents) {
	...
	{
		console.log("no registerGame"); return;
		const collection = this.collection(GAMES_COLLECTION_NAME);
		...
		if (game.playtime_2weeks)
		{
			...
			debug("'%s' has been recently played for %i minutes");
	...
	{
		console.log("no updateGamePlaytime"); return;
		const collection = this.collection(GAMES_COLLECTION_NAME);
	...
	{
		console.log("no updatePlayer"); return;
		const collection = this.collection(PLAYER_COLLECTION_NAME);
	...
	{
		console.log("no setGameAchievementGlobalPercent"); return;
	...
	{
		console.log("no setGameResynchronizationTime"); return;
		const collection = this.collection(GAMES_COLLECTION_NAME);

		debug("Update game resynchronized time", id);
		return collection.update({
			_id: id
		}, {
			$set: {
	...
	{
		console.log("no setGameAchievementAchieved"); return;
		const collection = this.collection(GAMES_COLLECTION_NAME);
	...
	{
		console.log("no setPerfectGame"); return;
		const collection = this.collection(GAMES_COLLECTION_NAME);
