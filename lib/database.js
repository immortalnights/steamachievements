...
module.exports = class Database {
	...
	getPlayerRecentGames(playerId, limit)
	{
		...
		const aggregate = [{
			$match: {
				...
				'owners.playtime_2weeks': { $not: { $size: 0 } }
			}
		},
		{
			$sort: {
		...
		},
		{
			$project: {
				...
				name: 1,
				'owners.lastPlayed': 1,
				'owners.playtime_forever': 1,
				'owners.playtime_2weeks': 1,
				img_icon_url: 1,
				img_logo_url: 1
			}
		}];
	...
	getPlayerRecentAchievements(playerId)
	{
		const collection = this.collection(GAMES_COLLECTION_NAME);
		...
		.sort(sort)
		.limit(3).toArray().then(function(documents) {
			...
			_.each(documents, function(doc) {
				_.each(doc.achievements, function(achievement) {
					...
					if (players && players[playerId])
					{
						...
						keep.splice(0, Math.max(0, keep.length - 3));
					}
				});
			});
