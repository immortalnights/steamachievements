...
module.exports = class Game {
	...
	canResynchronize()
	{
		...
		{
			debug("Game '%s' resynchronization requested");
			ok = true;
		}
		else if (!this.attr.resynchronized || this.attr.resynchronized === 'never')
		{
			debug("Game '%s' has never been resynchronized", this.attr._id);
			ok = true;
		}
		else
		{
			...
			debug("Game '%s' last resynchronized at %s (due %s)", this.attr._id, lastResynchronized, due);
			if (ok)
			{
				debug("Will resynchronize game '%s' now", this.attr._id);
			}
			else
			{
				debug("Will not resynchronize game '%s'", this.attr._id);
			}
		}
