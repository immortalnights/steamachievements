...
module.exports = function(db, steam) {
	...
	router.route('/Players/:id')
		.get((req, res) => {
			res.set('Last-Modified', moment(req.player.resynchronized).toDate().toUTCString());
			res.set('Cache-Control', 'maxAge=86400');
			res.send(req.player);
		});
	...
	router.route('/Players/:id/Resynchronize/invoke')
		.put((req, res) => {
			...
			console.log("force=", (req.query['force'] === 'true'));
		...
		.get((req, res) => {
			...
			.then(function(gameSummary) {
				...
				.then(function(achievementSummary) {
					...
					console.log("get player recent games", playerId);
					return db.getPlayerRecentGames(playerId)
					.then(function(recentGames) {

						console.log("get player recent achievements", playerId);
						return db.getPlayerRecentAchievements(playerId)
						.then(function(recentAchievements) {
							res.set('Last-Modified', moment(req.player.resynchronized).toDate().toUTCString());
							res.set('Cache-Control', 'maxAge=86400');
							res.send(_.extend({}, {
								id: playerId,
